...
- [ ] (відкладено — потрібна перевірка на живому пристрої) **Binary sensor `ground` інвертований** — `device_class=SAFETY` означає on=Unsafe, а код вмикає сенсор при `ground == 1` (`binary_sensor.py`, `_ACTIVE_VALUE`). За API-докою (eveus-api-doc) `ground: 0/1 = Ground present`, і в обох mockData-снапшотах (Bolt V1, VW V2) робочий стан — `ground: 1`. Тобто у нормальній роботі сенсор постійно показує «Unsafe». Фікс: `raw_on = (ground == 0)` (немає землі → небезпечно); підтвердити на живому пристрої.
